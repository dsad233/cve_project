        ...
        if(data.ok){
            try {
                ...
                worksheet.columns = [
                    ...
                    { header : "취약점 릴리즈 날짜", key : "cve_pushed", width : 20 },
                    { header : "취약점 최종 수정 날짜", key : "cve_lastModified", width : 20 },
                ...
                res.setHeader(
                  "Content-Disposition",
                  "attachment; filename=" + "cveIdSearch" + currentDayFormat + "_" + "excelfile.xlsx"
                );
...
// keyword를 통한 엑셀 다운로드
app.get('/search/excel/:keyword', async (req, res) => {
    try {
        const { keyword } = req.params;
        const cveApiUrl = `https://services.nvd.nist.gov/rest/json/cves/2.0?keywordSearch=${keyword}`;
        const data = await fetch(cveApiUrl);
        ...
        if(data.ok){
            try {
                ...
                for(let i = 0; i < sliceData.length; i++){
                    cveData.push({
                        cve_Id : sliceData[i].cve.id,
                        cve_source : sliceData[i].cve.sourceIdentifier,
                        cve_pushed : sliceData[i].cve.published,
                        cve_lastModified : sliceData[i].cve.lastModified,
                        cve_status : sliceData[i].cve.vulnStatus,
                    });
                }

                worksheet.addRows(cveData);
                ...
                res.setHeader(
                  "Content-Disposition",
                  "attachment; filename=" + "keywordSearch" + currentDayFormat + "_" + "excelfile.xlsx"
                );
